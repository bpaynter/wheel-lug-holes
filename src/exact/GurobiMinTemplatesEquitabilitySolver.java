 ...
 *
 */
public class GurobiMinTemplatesEquitabilitySolver {
	/**
	 * TODO
	 */
	 ...
	 *
	 */
	public GurobiMinTemplatesEquitabilitySolver() {
		try {
	...
	*/
	...
	public void setPatterns(PatternSet patterns, int upperBound) {
		...
		try {
			...
			min = model.addVar(0, patterns.numPatterns(), -1, GRB.CONTINUOUS, "Min Template Size.");
			max = model.addVar(0, patterns.numPatterns(), 1, GRB.CONTINUOUS, "Max Template Size.");
			model.update();
			for (int i = 0; i < patterns.numPatterns(); i++) {
				for (int j = i+1; j < patterns.numPatterns(); j++)
					for (int t = 0; t < upperBound; t++) {
			...
			for (int t = 0; t < upperBound; t++) {
				...
				GRBLinExpr rhs = new GRBLinExpr();
				rhs.addTerm(1.0, min);
				model.addConstr(lhs, GRB.GREATER_EQUAL, rhs, "Min" + t);
				rhs = new GRBLinExpr();
				rhs.addTerm(1.0, max);
				model.addConstr(lhs, GRB.LESS_EQUAL, rhs, "Max" + t);
			}
		} catch (GRBException e) {
			System.err.println("Error Code: " + e.getErrorCode() + ". " + e.getMessage());
		}
